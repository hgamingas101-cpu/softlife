...
// LÓGICA INTERACTIVA PARA LA SECCIÓN DEL MANUAL
document.addEventListener("DOMContentLoaded", function () {
    const btnMostrarManual = document.getElementById('btnMostrarManual');
    const manualQrArea = document.getElementById('manualQrArea');

    if (btnMostrarManual && manualQrArea) {
        btnMostrarManual.addEventListener('click', function () {
            // Si está oculto lo muestra, si está visible lo oculta de nuevo
            if (manualQrArea.style.display === 'none') {
                manualQrArea.style.display = 'block';
                btnMostrarManual.textContent = '❌ Ocultar Acceso al Manual';
            } else {
                manualQrArea.style.display = 'none';
                btnMostrarManual.textContent = '🔗 Generar Acceso al Manual';
            }
        });
    }
});